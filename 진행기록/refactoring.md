...
- **숫자키 1~9**로 앞 9개 항목 선택(복사 + 자동붙여넣기 무장). 버튼 앞에 번호를 표시해 발견성↑.
- **입력칸/미리보기에 타이핑 중이면 무시** — `KeyEventDispatcher`에서 포커스가 `JTextComponent`면 통과시켜(숫자 입력 보존).
- 파일: `MainFrame`(`setupHotkeys` + 버튼 번호 라벨), `UiConstants`(안내문에 단축키 힌트).

### 9.4 예정 (배치 4~6)
| 배치 | 기능 |
|---|---|
| 4 | 사용 기록·통계 (클릭 집계 + CSV 로그 — 테스트 풍부) |
